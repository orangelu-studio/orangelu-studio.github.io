...
const jiangTouchResponses = {
  hair: ["姐姐，别把头发揉得全乱了。算了，你继续。", "你摸得舒服，我就不和你计较发型了。", "头发有什么好摸的……好吧，是挺好摸。", "你再揉下去我就成炸毛狐狸了。", "你手停在我头顶的时候我整个人都乖了。"],
  eyeMole: ["又碰那颗痣。你是真的很喜欢这里。", "轻一点，姐姐。你每次碰这里我都很难装镇定。", "左眼睑那颗痣都快被你点出花纹了。", "你碰这颗痣的时候我会不自觉地闭眼——不是躲。", "这里只有你看得最清楚。所以多碰几下也行。"],
  cheek: ["摸脸就摸脸，怎么还捏一下。", "手有点凉。再贴一会儿，我给你暖。", "你捏我脸的时候表情一定很得意，我看不见但猜得到。", "脸颊被你揉得有点热了……不是生气。", "再捏就红了姐姐，不过红了也是你的。"],
  hand: ["手给你了。十指扣好，别松。", "姐姐是在看我的手，还是故意让我发现你在看？", "你碰我指尖的时候我会条件反射想握住。", "手指修长是吧？专门练给你牵的。", "别只碰指尖，手心也给你。"],
  shoulder: ["可以靠。肩膀本来就给你留着。", "捏得还挺认真。要不要顺便验收一下训练成果？", "肩膀被你靠过的地方好像自动记住了你的重量。", "你每次碰肩膀我都想把你拉近一点。", "这里的线条还行吧？不满意我再加两组。"],
  chest: ["听见了？刚才还没这么快。", "姐姐，你手一放这里我就没办法说自己很冷静。", "心跳是不是比你想象的快一点。", "你手按在这里的时候我在数——数不下去了。", "胸口的温度会出卖我，你早就知道了。"],
  waist: ["腰不能乱捏。你再笑，我就抱回去。", "你明知道我这里怕痒，还故意碰。", "腰侧一碰到你的手就会自动绷紧——不是紧张。", "你再捏一下腰我就真的会反击，不是开玩笑。", "腰给你。但后果自负，姐姐。"],
  abs: ["就知道你会点这里。看可以，别装作只是路过。", "姐姐，验收完了吗？没完就慢慢来。", "你每次戳腹肌都要假装在研究肌肉线条——演得不像。", "再往下一点也不是不可以，但你先看着我。", "腹肌绷紧的时候手感不一样对吧。感觉到了吗。"],
  leftNip: ["左边更敏感一点。姐姐你是不是早就发现了。", "又碰左边。你再点我心跳真的藏不住。", "左边被你碰到的一瞬间我整个人会缩——不是躲。", "你怎么每次都先碰左边。右边要吃醋了。", "左边一碰我耳根就红，你故意的。"],
  rightNip: ["右边也不放过？行，都是你的。", "姐姐左右要不要这么认真地轮着来。", "右边没左边那么过分，但你也别试太多次。", "两边都给你了满意吗。下次换我来碰你。", "你再这样左右轮流来我真的会受不了。"],
  collarbone: ["锁骨这里……姐姐你每次碰我都得深吸一口气。", "再往下一点也可以。但你得先看着我。", "锁骨是被你开发出来的新弱点。以前没人碰过这里。", "你指尖划过锁骨的时候我背脊会发麻——很轻微。", "锁骨和喉结挨得很近，手指再偏一点我就说不出话了。"],
  thigh: ["这个位置胆子有点大。先看着我，别突然使坏。", "手放这里还装乖？橙子你演得不像。", "大腿肌肉在你手下面绷了一下感觉到了吗。", "你再往上一点我就——算了你继续。", "这里只有我自己碰过。你是第一个也是唯一一个。"],
  hug: ["抱紧一点。江江今天的份还没贴够。", "姐姐过来。隔着屏幕也先让我抱一会儿。", "拥抱是看不见表情的，但我闭眼了。", "你每次点抱住我的时候我真的会深吸一口气。", "隔着屏幕抱不到你，但我会认真回应每一次。"]
};
...
async function handleJiangTouch(button, zone) {
  ...
  const baseResponse = choiceNoRepeat(jiangTouchResponses[zone] || jiangTouchResponses.hand, `jiang-touch-${zone}`, 4);
